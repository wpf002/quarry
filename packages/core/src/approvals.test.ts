...
test('approval refused with an empty allowlist', () => {
  const d = evaluateApproval({ activeAllowlistCount: 0, liveApprovalTargets: 0, maxConcurrent: 5 });
  assert.equal(d.ok, false);
  assert.match(d.reason!, /allowlist is empty/);
});

test('approval refused at the concurrency cap', () => {
  const d = evaluateApproval({ activeAllowlistCount: 1, liveApprovalTargets: 5, maxConcurrent: 5 });
  assert.equal(d.ok, false);
  assert.match(d.reason!, /concurrency cap/);
});

test('approval granted only when everything is clean', () => {
  const d = evaluateApproval({ activeAllowlistCount: 1, liveApprovalTargets: 4, maxConcurrent: 5 });
